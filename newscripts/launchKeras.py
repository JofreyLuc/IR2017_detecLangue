...
def formatData(dataFilePath, withoutLabel=False, hdf5TmpPath=None):
    '''
    ...
    '''
    hdf5In = h5py.File(dataFilePath, "r")
    ...
    if hdf5TmpPath is not None: # création des datasets dans le fichier hdf5 temporaire si nécessaire
        # On places les datasets générés par cette fonction dans des groupes nommés de 1 à n
        
        hdf5Tmp = h5py.File(hdf5TmpPath, "w")
        groupLastName = 0
        ...
        hdf5Tmp.create_dataset(newGroupName+"/examples", (totalFrames, nbVal * nbCoef), dtype=np.float32) # dataset données
        X = hdf5Tmp.get(newGroupName+"/examples")
        if not withoutLabel:
            hdf5Tmp.create_dataset(newGroupName+"/languages", (totalFrames, 4), dtype=np.float32, fillvalue=0)           # dataset étiquettes
            Y = hdf5Tmp.get(newGroupName+"/languages")
    else :  # sinon utilisation de tableau numpy
...
if __name__ == '__main__':
    ...
    (Xdev, Ydev) = formatData(devFileName, hdf5TmpPath="de.hdf5")
    (Xtest, Ytest) = formatData(testFileName, hdf5TmpPath="te.hdf5")
    ...
    for i in range(len(X)):
        X[i] /= variancesDuplicate

    for i in range(len(Xdev)):
        Xdev[i] /= variancesDuplicate
